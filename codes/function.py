...
from Aindexdata_onlyfinalcluster import valuescollection,valuescollection_G1
...
def values_all(A, B, C, D, E, F, G):
    ...
    def find_first_small_col(df):
        ...
        for i, col in enumerate(df.columns):
            if (df.loc[:, col] < 0.005).all():
                return i
        return df_NIN_mean_change.shape[1]-1
    list2 = []
    ...
    list2 = []
    list2 = ['NIN_clusters','NICN_clusters','NIC_clusters','all_clusters']
    for result in list2:
        globals()[f"{result}"] = 0
...
for C in [i/10 for i in range(11)]:
    for D in [i/10 for i in range(11)]:
        for E in [i/10 for i in range(11-int(10*D))]:
            F = round(1 - D - E, 1)
            for G in [i/10 for i in range(-10, 12, 2)]:
